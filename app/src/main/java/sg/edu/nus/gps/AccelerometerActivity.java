...
import static sg.edu.nus.gps.R.id.accLocNumber;

public class AccelerometerActivity extends AppCompatActivity implements SensorEventListener, ConnectionCallbacks,
    ...
    //xml variables
    TextView number_of_loc;
    DataPoint center;
    Vector<DataPoint> gps_locations, fixed_estimate;
    TextView x, y, degreeView;
    double degree;
    ...
    public void accStart() {
        ...
        sensorReading = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    number_of_loc = (TextView) findViewById(accLocNumber);
                    x = (TextView) findViewById(R.id.textX);
                    y = (TextView) findViewById(R.id.textY);
                    degreeView = (TextView) findViewById(R.id.northPos);
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            x.setText("North Acc: ");
                            y.setText("East Acc: ");
                            degreeView.setText("Orientation: ");
            ...
            public void sensorReading() {
                if (event != null) {
                    yVal = Math.cos(degree_to_rad(degree)) * Math.sqrt((event.values[0]*event.values[0])
                            + (event.values[1]*event.values[1]));
                    xVal = Math.sin(degree_to_rad(degree)) * Math.sqrt((event.values[0]*event.values[0])
                            + (event.values[1]*event.values[1]));
                    ...
                    if (mLastLocation != null) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                x.setText("East Acc: " +
                                        Double.parseDouble(new DecimalFormat("##.####").format(xVal)) + "m/s^2");
                                y.setText("North Acc: "
                                        + Double.parseDouble(new DecimalFormat("##.####").format(yVal)) + "m/s^2");
                                degreeView.setText("Orientation: "
                                        + Double.parseDouble(new DecimalFormat("##").format(degree)) + "\u00b0");
                                rowText.setText("North Acc: "
                                        + Double.parseDouble(new DecimalFormat("##.####").format(yVal)) + "m/s^2" + ", East Acc: "
                                        + Double.parseDouble(new DecimalFormat("##.####").format(xVal)) + "m/s^2");
                                rowText1.setText("Orientation: "
                                        + Double.parseDouble(new DecimalFormat("##").format(degree)) + "\u00b0");
                        ...
                        if (first == true) {
                            ...
                            fixed_estimate.add(new DataPoint(xLong, yLat));
                            first = false;
                        } else {
                            gps_locations.add(new DataPoint(xLong, yLat));
                            int i = fixed_estimate.size();

                            double xLongEst = fixed_estimate.get(i-1).getX() - ((0.5)*xVal);
                            double yLatEst = fixed_estimate.get(i-1).getY() + ((0.5)*yVal);
                            fixed_estimate.add(new DataPoint(xLongEst, yLatEst));
                        }
                    }
                }
            }
    ...
    public void accStop(View view){
        ...
        myIntent = new Intent(this, AccGraphActivity.class);
        //add the 2 extras and start
        myIntent.putExtra("gps", gps_locations);
        myIntent.putExtra("fixed", fixed_estimate);
